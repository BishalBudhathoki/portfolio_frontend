...
async function getBlogsData() {
  try {
    ...
    // Use two different URL formats:
    // 1. Direct to backend if NEXT_PUBLIC_API_URL is set
    // 2. Relative URL (/api/profile) which will go through Next.js rewrites if on same domain
    const directBackendUrl = apiUrl ? `${apiUrl}/api/profile?t=${timestamp}` : null;
    const relativeUrl = `/api/profile?t=${timestamp}`;
    
    // Try the relative URL first (works with custom domains through rewrites)
    const urlToUse = relativeUrl;
    console.log("Blogs page - Using URL:", urlToUse);
    ...
    try {
      const data = await fetchWithValidation<ApiResponse>(urlToUse, {
        next: { revalidate: 0 },
        cache: 'no-store'
      }, (data) => {
